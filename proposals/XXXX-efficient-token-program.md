...
About `~10%` of block compute units is used by the Token program instructions. Decreasing the CUs used by Token program instruction creates block space for other transactions to be executed, i.e., less CUs consumed by Token program, more CUs for other transactions.
...
`p-token` must be guaranteed to follow the same instructions and accounts layout, as well as have the same behaivour than the current Token implmentation.

Any potential risk will be mitigated by extensive fixture testing, formal verification and audits.
